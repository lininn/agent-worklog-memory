...
`{{AI_MEMORY_HOME}}/projects/<project-name>/worklog/YYYY-MM.md`

Use the current Git repository root directory name as `<project-name>`; outside Git, use the current working directory name. Every worklog entry must include the current branch from `git branch --show-current`; for detached HEAD, record `detached:<short-sha>`, and outside Git record `无 Git 仓库`.
